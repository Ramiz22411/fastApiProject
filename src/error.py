...
class AccessTokenRequired(BooklyException):
    """User has provided a refresh token when a refresh token is needed"""
    pass
...
class UserAlreadyExists(BooklyException):
    """User has provided an email address that already exists"""
    pass


class InvalidCredentials(BooklyException):
    """User has provided an email for user who exists during log in"""
    pass


class InsufficientPermissions(BooklyException):
    """User does not have the necessary permissions to perform the action"""
    pass
...
def create_exception_handler(status_code: int, initial_detail: Any) -> Callable[
    [Request, Exception], JSONResponse]:
...
def register_all_error(app: FastAPI):
    ...
    app.add_exception_handler(BookNotFound, create_exception_handler(
        status_code=status.HTTP_404_NOT_FOUND,
        initial_detail={
    ...
    app.add_exception_handler(RevokedToken, create_exception_handler(
        status_code=status.HTTP_401_UNAUTHORIZED,
        initial_detail={
    ...
    app.add_exception_handler(InsufficientPermissions, create_exception_handler(
        status_code=status.HTTP_401_UNAUTHORIZED,
        initial_detail={
    ...
    app.add_exception_handler(TagNotFound, create_exception_handler(
        status_code=status.HTTP_404_NOT_FOUND,
        initial_detail={
    ...
    app.add_exception_handler(TagAlreadyExists, create_exception_handler(
        status_code=status.HTTP_403_FORBIDDEN,
        initial_detail={
    ...
    app.add_exception_handler(AccountNotVerified, create_exception_handler(
        ...
        initial_detail={
            ...
            "resolution": "Please check your email for verification0",
        },
    ), )
